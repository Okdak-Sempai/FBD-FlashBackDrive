...
WCHAR* intToWCHAR(int number)
/*
* Includes:
*   wchar.h [swprintf(); _wcsdup()]
* Params:
*   int number = The number to convert
* Return:
*   WCHAR* => The WCHAR of the number
* Description:
*   This function converts an int to a WCHAR* and returns it
* Notes:
*   None
*/
{
    WCHAR buffer[20];
    swprintf(buffer, sizeof(buffer) / sizeof(buffer[0]), L"%d", number);
    return _wcsdup(buffer); // Allocate and return a copy of the buffer
}

WCHAR* getWCHARString(WCHAR* message, int maxLength)
/*
* Includes:
*   stdio.h
*   stdlib.h
*   wchar.h
* Params:
*   WCHAR* message = The message to print to get the input
*   maxLength = The maximum length of the input
* Return:
*   WCHAR* => The WCHAR* that got inputed, all type compatibles
* Description:
*   This function write the message with a limit of maxLenght and returns it
* Notes:
*   /!\ The return has to be free() /!\
*   The \n has to be included in the message if needed
*/
{
    // Allocate memory for the wide character buffer
    WCHAR* buffer = (WCHAR*)malloc((maxLength + 1) * sizeof(WCHAR));
    ...
    // Use fgetws to read a line of input
    if (fgetws(buffer, maxLength + 1, stdin) != NULL)
    {
