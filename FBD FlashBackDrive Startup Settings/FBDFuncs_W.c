...
*   FoldersFuncs_W.h/.c {currentDirectory()}
*   FilesFuncs_W.h/.c {writeLineNumber(); readLineNumber()}
*   FBDFuncs_W.h/.c {allDrivesToggle()}
* Params:
*   char* settingsfilename = Name of the FBD Default Backups; waits for "FBDdrivesSettings" string or "FBDdrivesSettings" string
* Return:
*   int
*       1 => The file already exists and is not NULL inside
*       0 => The file was created sucessfully
*       -1 => Error creating this file
* Description:
*   This function initialize the FBDdrivesSettings file at the root of the Software/Code (If it already didn't exists)
* Notes:
*   This will automatically exclude the C drive
*   This checks if the path is NULL, and if so it will write the default back(Still without the C letter)
*/
{
    FILE* file = NULL;
    ...
    if (err == 0 && file != NULL)
    {
        size_t requiredSize = wcslen(alldrives) + 1; // +1 for null character
        // Allocate full memory for the full path
        WCHAR* fullPath = (WCHAR*)malloc(requiredSize * sizeof(WCHAR));
        if (fullPath != NULL)
        {
            // Copy existing path to new path
            wcscpy_s(fullPath, requiredSize, alldrives);
        }
        else
        {
            wprintf(L"Erreur lors de l'allocation de mémoire.\n");
        ...
        free(fullPath);
        fclose(file);
        allDrivesToggle(L"C");//Remove the C drive by default
        return 0; // Create the file sucessfully
    }
    else
    {
        fprintf(stderr, "Impossible to create FBDdrivesSettings file.\n");
        return -1; // Creation error
    }
...
*   Tools_W.h/.c {choiceBTW(); getWCHARString()}
*   FBDFuncs_W.h/.c {allDrivesToggle()}
* Params:
*   char* settingsfilename = The name of the settings file
* Return:
*   int:
*       00 => The path got back to the defaultPath successfully
*       02 => The path got changed successfully
*
*       10 => The default drives settings got back succesfully
*       12 => The default drives settings got changed successfully
*
*
*       1 => The path got kept successfully
*       -1 => failed to open the file
* Description:
*   This function is a complete menu to manage the setting file or the drives Settings.
*   For the defaultPath Settings 0 and drives Settings 1
*
*   For the defaultPath Settings
*       Choice 0: To set back the defaultPath
*       Choice 1: To keep the path
*       Choice 2: To change the path
*
*   For the drives Settings
*       Choice 0: To set back the the Settings back to default
*       Choice 1: To keep the Settings as it is
*       Choice 2: To toggles drives
* Notes:
*   If a change is made but nothing is choosen for the defaultPath, it will put the default path back automatically
*/
{
    system("cls");
    ...
    //Print the FBDdrivesSettings content

    FILE* fileTwo = NULL;
    int errTwo = fopen_s(&fileTwo, "FBDdrivesSettings", "r");
    // Existing check
    if (errTwo != 0 || file == NULL)
    {
        // File couldn't be opened
        wprintf(L"Failed to open the file: %s\n", "FBDdrivesSettings");
        return -1;
    }
    WCHAR* firstLineAD = readLineNumber(fileTwo, 1);
    wprintf(L"\nThe current Settings for all drives are :\n[%ls]\n ", firstLineAD);
    fclose(fileTwo);

    // Put back the defaultPath OR keep the current one OR Change to another file

    int choicedrives = choice(L"To change the defaultPath Settings do", 0, L"To change the Settings for all drives do", 1);
    if (choicedrives == 0)//defaultPath settings
    {
        int choice = choiceBTW(L"To set back the defaultPath do", 0, L"To keep the path do: 1\nTo change the path do", 2);
        //Choice 1 skips all obviously
        if (choice == 2)
        {
            err = fopen_s(&file, settingsfilename, "w");
            if (err != 0 || file == NULL)
    ...
    else //all drives settings
    {
        wprintf(L"Here is the list of active path, The dots are the Inactive path\n");
        wprintf(L"You can let unused drives activated it won't cause any harm or bug! So one's might only want to rend inactive the drives to be excluded\n");
        wprintf(L"The C drive is excluded by default but can be toggled back if needed\n");

        wprintf(L"\nThe current Settings for all drives are :\n[%ls]\n", firstLineAD);

        int choice = choiceBTW(L"To set back the the Settings back to default do", 0, L"To keep the Settings as it is do: 1\nTo toggles drives do", 2);
        //Choice 1 does then notthing
        if (choice == 0)//Put default Drives
        {
            FILE* fileDrive = NULL;
            err = fopen_s(&fileDrive, "FBDdrivesSettings", "w");
            if (err == 0 && fileDrive != NULL)
            {
                size_t requiredSize = wcslen(alldrives) + 1; // +1 for null character
                // Allocate full memory for the full path
                WCHAR* fullPathe = (WCHAR*)malloc(requiredSize * sizeof(WCHAR));
                if (fullPathe != NULL)
                {
                    // Copy existing path to new path
                    wcscpy_s(fullPathe, requiredSize, alldrives);
                }
                else
                {
                    wprintf(L"Erreur lors de l'allocation de mémoire.\n");
                }
                writeLineNumber(fileDrive, 1, fullPathe);
                free(fullPathe);
                fclose(fileDrive);
                allDrivesToggle(L"C");//Remove the C drive by default
                ...
                //free(firstLine);
                //free(firstLineAD);
                return 10; // Create the file sucessfully
            }
            else
            {
                fprintf(stderr, "Impossible to create FBDdrivesSettings file.\n");
        ...
        if (choice == 2)//Toggle Drives
        {

            WCHAR* userResult = getWCHARString(L"Enter all the letters you want to toggle and press enter: ", 30);
            allDrivesToggle(userResult);
    ...
    {
        wprintf(L"\nThe current drive Settings is now:\n[%ls]\n ", firstLineAD);
    }
    free(firstLine);
    free(firstLineAD);
    return 1;
...
*   WCHAR* => The path stored in the first line of the Settings file
* Description:
*   This function prints the the Settings for all drives and returns it
*   Ex:
*       The current default Path is :
*       [C:\Users\Steve\source\repos\Safe file\FBD Default Backups]
* Notes:
...
*/
{
    ...
    WCHAR* firstLine = readLineNumber(file, 1);
    wprintf(L"\nThe current default Settings for all drives are :\n[%ls]\n", firstLine);
    fclose(file);
    return firstLine;
}
...
*   FoldersFuncs_W.h/.c {countFilesInFolder(); createFolder(); finalPathFileExplorer()}
*   FilesFuncs_W.h/.c {getFilename()}
*   FBDFuncs_W.h/.c {createBackupFileRegisterOnly(); fileExplorer()}
* Params:
*   WCHAR* Real_filepathInit = The path to explore
*   WCHAR* filepathInit = The path to explore with the L"\\*"
*   WCHAR* Real_filepathDEST = The destination path
...
*   DWORD rootsize = Number of folders in filepathInit
*   DWORD pathsize = Number of folders in filepathDEST
*   WCHAR* usedPATH = The path of the root of the folder destination
*   const WCHAR* registerAdress = The adress of the register(The path)
*   WCHAR* extraExcludePath = The Second path to exclude
* Return:
*   None
* Description:
*   This function parse the folder as filepathInit and make a copy of it to filepathDEST only in the Register
*   There is a recursion so to function can act by going as deep as possible in the folders while creating the files and proceed to create the copy of the files
* Notes:
*   Be carefull of the arguments so the recursion goes well
*   This will ignore all path that has "FBD FlashBackDrive"
    ...
    if (hFind != INVALID_HANDLE_VALUE)
    {
        do
        {
            if (!(FileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) // For files
            {
                ...
                pathsize = countFilesInFolder(usedPATH) - 1; //Do not count the register
                //wprintf(L"\rProgression: %d%%", (int)(((float)pathsize / rootsize) * 100));
                wprintf(L"\rProgression: ...");


                // Call createBackupFile() only for files, not folders
                createBackupFileRegisterOnly(1, fullPath, Real_filepathDEST, registerAdress, &line);
            }
            else // For folders
            {
                if (wcscmp(FileData.cFileName, L".") != 0 && wcscmp(FileData.cFileName, L"..") != 0)
                {
                    ...
                    if ((wcsstr(fullPath, pathToIgnore) == NULL) && (wcscmp(extraExcludePath, fullPath) != 0)) //Ignore paths
                    {
                        ...
                        fileExplorerRegisterOnly(initDEST, star_initDEST, newDEST, star_newDEST, rootsize, pathsize, usedPATH, registerAdress, extraExcludePath);
                        free(initDEST);
                        free(star_initDEST);
                        free(newDEST);
                        free(star_newDEST);
...
*   FilesFuncs_W.h/.c {readLineNumber()}
* Params:
*   WCHAR* changeArray = An array that contains the letters to toggle
* Return:
*   int
*       -1 => Error opening the file
*       0 => Sucess
* Description:
*   This function Toggle the drives settings using the chageArray letters. (In the file)
*   This either put a L'.' or the letter back
* Notes:
*   Prints nothing
*/
{
    ...
    FILE* file = NULL;
    int err = fopen_s(&file, "FBDdrivesSettings", "r+");
    // Existing check
    if (err != 0 || file == NULL)
    {
        // File couldn't be opened
    ...
    WCHAR* firstLineAD = readLineNumber(file, 1);

    int changeArraysize = wcslen(changeArray);
    int alldrivessize = sizeof(alldrives) / sizeof(alldrives[0]) - 1;
    //Change the first line
    for (int i = 0; i < changeArraysize; i++)
    {
        for (int j = 0; j < alldrivessize; j++)
        {
            WCHAR test = toupper(changeArray[i]);

            if (toupper(changeArray[i]) == alldrives[j])
            {
                if (firstLineAD[j] == alldrives[j])
                {
                    firstLineAD[j] = L'.';
                }
                else
                {
                    firstLineAD[j] = toupper(alldrives[j]);
                }
            }
        }
    }

    //Modify FBDdrivesSettings
    writeLineNumber(file, 1, firstLineAD);
    free(firstLineAD);
    if (fclose(file) != 0)
    {
        wprintf(L"Failed to close the file %s\n", "FBDdrivesSettings");
        return -1;
    }
    return 0;
}